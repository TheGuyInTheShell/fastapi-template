...
async def init_auth(permissions_routes: List[Dict[str, Any]], sessionAsync: AsyncSession):
    for permission_routes in permissions_routes:
        await create_permissions_api(permission_routes['routes'], sessionAsync, permission_routes['type'])
